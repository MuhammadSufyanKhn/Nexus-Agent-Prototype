...
import { Sparkles, CornerDownLeft, ArrowUpDown, X, Zap, BookOpen } from 'lucide-react';
    ...
    <div className="absolute left-0 right-0 top-full mt-2 bg-slate-950/95 text-white rounded-2xl shadow-2xl border border-indigo-500/40 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150 backdrop-blur-xl">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 border-b border-indigo-500/30 text-[11px]">
        <div className="flex items-center gap-2 font-bold">
          ...
          <span className="text-white font-extrabold tracking-wide">AI Assistant &amp; Guide Suggestions</span>
      ...
      <div className="p-2 space-y-1.5 max-h-72 overflow-y-auto custom-scrollbar">
        {suggestions.map((sug, idx) => {
          ...
          const isInstruction = sug.isInstructionGuide || sug.command.isInstructionGuide;
          const isGeminiSuggestion = !isInstruction && (sug.matchedBy === 'prefix' || sug.command.category.includes('Gemini'));

          const categoryColor = isInstruction
            ...
            >
              <div className="space-y-1 min-w-0 pr-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isInstruction ? (
                    <span className="text-[9px] bg-indigo-950/80 text-indigo-300 border border-indigo-500/50 font-mono px-2 py-0.5 rounded-full flex items-center gap-1 font-bold">
